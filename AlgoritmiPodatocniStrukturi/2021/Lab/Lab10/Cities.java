...
class Graph<E> {
    ...
    float[] dijkstra(int from, int[] path) {
    ...
    public int getIndexFromValue(E val){
...
public class Cities {
    public static void main(String[] args) throws IOException {
        ...
        int from = graph.getIndexFromValue(bf.readLine());
        int to = graph.getIndexFromValue(bf.readLine());
        int[] path = new int[N];
        ...
        ss.push(from);
        while (!ss.isEmpty()) System.out.print(graph.adjList[ss.pop()].getInfo()+" ");
        System.out.println();
        curr = from;
        length += graph.dijkstra(to,path)[from];
        while (curr!=to){
            ss.push(curr);
            curr = path[curr];
        }
