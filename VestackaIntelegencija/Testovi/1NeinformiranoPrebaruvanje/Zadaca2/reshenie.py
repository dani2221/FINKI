...
class Disks(Problem):
    ...
    def __init__(self, initial, goal, n, l):
    ...
    def successor(self, state):
        # state -> (1,2,0,0,...,n)
        succ = dict()
        for index_element, element in enumerate(state):
            if element == 0:
                continue
            if index_element+1 < self.l and state[index_element + 1] == 0:
                swap = list(state)
                swap[index_element], swap[index_element + 1] = swap[index_element + 1], swap[index_element]
                succ[f"D1: Disk {element}"] = tuple(swap)
            if index_element+2 < self.l and state[index_element+1] != 0 and state[index_element+2] == 0:
                swap = list(state)
                swap[index_element], swap[index_element + 2] = swap[index_element + 2], swap[index_element]
                succ[f"D2: Disk {element}"] = tuple(swap)
            if index_element-1 >= 0 and state[index_element - 1] == 0:
                swap = list(state)
                swap[index_element], swap[index_element - 1] = swap[index_element - 1], swap[index_element]
                succ[f"L1: Disk {element}"] = tuple(swap)
            if index_element-2 >= 0 and state[index_element - 1] != 0 and state[index_element - 2] == 0:
                swap = list(state)
                swap[index_element], swap[index_element - 2] = swap[index_element - 2], swap[index_element]
                succ[f"L2: Disk {element}"] = tuple(swap)
        return succ
...
if __name__ == "__main__":
    ...
    initial.reverse()
    end_state = tuple(initial)
    problem = Disks(initial_state, end_state, n, l)
