    ...
    def successor(self,state):
        ...
        for i in range(self.n):
            for j in range(self.n):
                temp_list = [list(t) for t in state]
                temp_list[i][j] = not temp_list[i][j]
                if i-1 >= 0:
                    temp_list[i-1][j] = not temp_list[i-1][j]
                if i+1 < n:
                    temp_list[i+1][j] = not temp_list[i+1][j]
                if j-1 >= 0:
                    temp_list[i][j-1] = not temp_list[i][j-1]
                if j+1 < n:
                    temp_list[i][j+1] = not temp_list[i][j+1]
                back_to_tuple = [tuple(t) for t in temp_list]
                succ[f'x: {i}, y: {j}'] = tuple(back_to_tuple)
        return succ
...
if __name__ == '__main__':
    ...
    for i in range(n):
        ...
        temp_goal = list()
        for j in range(n):
            temp.append(fields[current] == 1)
            temp_goal.append(True)
            current += 1
        initial.append(tuple(temp))
        goal.append(tuple(temp_goal))

    problem = Flip(tuple(initial), tuple(goal), n)
