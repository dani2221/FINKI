...
class Snake(Problem):
    ...
    def successor(self, state):
        ...
        if d == 'dole':
            new_head_pos = (x,y-1)
            if y-1>=0 and new_head_pos not in positions[:-1] and new_head_pos not in self.crveni_jabolki:
                succ['ProdolzhiPravo'] = self.change_state(new_head_pos,apples,positions,d)

            new_head_pos = (x - 1, y)
            if x-1>=0 and new_head_pos not in positions[:-1] and new_head_pos not in self.crveni_jabolki:
                succ['SvrtiDesno'] = self.change_state(new_head_pos,apples,positions,'levo')

            new_head_pos = (x + 1, y)
            if x + 1 < 10 and new_head_pos not in positions[:-1] and new_head_pos not in self.crveni_jabolki:
                succ['SvrtiLevo'] = self.change_state(new_head_pos,apples,positions,'desno')

        elif d == 'gore':
            new_head_pos = (x,y+1)
            if y+1<10 and new_head_pos not in positions[:-1] and new_head_pos not in self.crveni_jabolki:
                succ['ProdolzhiPravo'] = self.change_state(new_head_pos,apples,positions,d)
...
if __name__ == '__main__':
    ...
    num_crveni = int(input())
    crveni = list()
    for i in range(num_crveni):
        crveni.append(tuple(map(int, input().split(','))))

    initial = ('dole',((0,7),(0,8),(0,9)),tuple(zeleni))

    problem = Snake(initial,tuple(crveni))
